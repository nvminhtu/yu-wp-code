            ...
            $.ajax({
               ...
               complete: function() {
                 ...
                 function setHeightBox(){
                   $('.alm-reveal').each(function(index, el) {
                     $item = $(this).find('.box_qa');
                     //$(this).find('.box_qa').eq(1).addClass('blue');
                     console.log(  $item.length);
                     for( var i = 0; i < $item.length; i++) {
                       console.log(i);
                       if(i%2==0) {
                         var heightThis = $(this).find('.box_qa').eq(i).height();
                         var heightNext = $(this).find('.box_qa').eq(i+1).height();
                         //console.log(heightNext);

                         if(heightThis<heightNext) {
                           $(this).find('.box_qa').eq(i).height(heightNext);
                         }

                         //console.log(heightCol);
                       } else {
                         var heightThis = $(this).find('.box_qa').eq(i).height();
                         var heightPrev = $(this).find('.box_qa').eq(i-1).height();
                         if(heightThis<heightPrev) {
                           $(this).find('.box_qa').eq(i).height(heightPrev);
                         }
                       }
                     }
                   });
