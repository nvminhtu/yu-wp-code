      ...
      public static function alm_render_shortcode($atts){
         ...
         if(self::$counter > 1){
   		...
   		// Retreive preloaded posts and append to ajax load more object
            ...
            $custom_arr = array( // Create preload data array
               'post_type'          => 'qa',
               'posts_per_page'     => 4
            );
            $type = alm_get_repeater_type($repeater);

            $alm_preload_query_1 = new WP_Query($custom_arr);
           // $output .= '<div class="box_list_qa clearfix">';
            ///--------------------Load pre-load posts - 4 posts load fixed -------------
            if ($alm_preload_query_1->have_posts()) :
               while ($alm_preload_query_1->have_posts()) : $alm_preload_query_1->the_post();
                  $output .= '<div class="box_qa heightLine-01 clearfix"><dl>';
                  $output .= '<dt>Q.'.get_the_title().'</dt>';
                  $output .= '<dd>A.'.get_the_content().'</dd>';
                  $output .= '</dl></div>';
            ...
            if(!$comments){
               ...
               $alm_preload_query = new WP_Query($custom_arr);
